...
fn generate_parity_inventory(options: GenerateParityInventoryOptions) -> Result<()> {
    ...
    if head != REFERENCE_COMMIT {
        return Err(format!(
            "upstream checkout at {} is {}; expected {}",
            options.repo.display(),
            head,
            REFERENCE_COMMIT
        )
    ...
    body.push_str(&format!("- Upstream commit: `{head}`\n"));
...
fn classify_inventory_path(path: &str) -> InventoryRow {
    ...
    } else if path == "apps/web/src/components/ui/button.tsx" {
        (
            "Button primitive data slot, default tag/type behavior, base class, default variant settings, size variants, and visual variant class matrix in crates/r3_core/src/lib.rs",
            "covered",
    ...
    {
        (
            "Small UI primitive data slots, class contracts, separator orientation default, and spinner icon/aria/role contract in crates/r3_core/src/lib.rs",
            "covered",
            "`cargo test --workspace small_ui_primitive_contracts_match_upstream_components`; current screenshot gates where captured",
            "Maintain these primitive contracts if upstream changes Kbd, Label, Separator, Skeleton, or Spinner styling.",
        )
