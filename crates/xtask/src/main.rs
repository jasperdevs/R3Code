...
fn print_usage() {
  ...
  cargo run -p xtask -- capture-r3code-window --allow-window-capture [--screen draft|active-chat|running-turn|pending-approval|pending-user-input|terminal-drawer|diff-panel|branch-toolbar|settings|command-palette|settings-theme-menu|settings-dark|settings-back|settings-keybindings|settings-source-control|settings-archive] [--theme light|dark|system] [--output <png>]
