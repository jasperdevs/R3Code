...
fn classify_inventory_path(path: &str) -> InventoryRow {
        ...
        || path == "packages/contracts/src/filesystem.ts"
    {
        ...
        (
            ...
            "Wire full live secret-store service permissions/concurrency layer, live repository-backed auth service execution, concrete HTTP auth exchange execution, websocket upgrade execution, live auth PubSub streams, and persisted runtime integration.",
        )
    } else if matches!(
        path,
