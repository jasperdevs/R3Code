...
fn classify_inventory_path(path: &str) -> InventoryRow {
    ...
    {
        (
            "transport-agnostic HTTP route plus attachment route decisions, static/dev redirect/path-guard/fallback/content-type helpers, browser API CORS constants and preflight/header decision contracts, project favicon route decisions, OTLP traces proxy route decisions, and WebSocket RPC method/group/schema/handler/dispatch contracts in crates/r3_core/src/rpc.rs plus crates/r3_core/src/server.rs",
            "partial",
            "`cargo test -p r3_core rpc`",
            "Port live HTTP server, WebSocket upgrade/auth handling, concrete handler execution, runtime schema decoding, subscriptions, actual static file reads, attachment filesystem response execution, live favicon file response execution, OTLP decode/record/export execution, and actual CORS middleware integration.",
        )
