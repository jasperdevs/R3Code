...
The Rust core also ports upstream `ChatView.logic.ts` and `terminalContext.ts` composer send contracts: inline terminal-context placeholders, terminal-context block append/extract/display state, expired terminal-context filtering, and expired-context toast copy.
