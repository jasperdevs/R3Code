...
| Settings providers/connections diagnostics depth | `settings/*`, `Provider*`, `ConnectionsSettings.tsx`, `ProcessDiagnostics.ts` | Partial provider, pairing, endpoint, process diagnostics, and diagnostics-format contracts + provider/connections smoke captures |
